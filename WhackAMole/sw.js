...
const CACHE_VERSION = 'whackmole-v1.6.0';
const ASSETS = [
  './',
  './index.html',
  './style.css',
...
self.addEventListener('fetch', (e) => {
  ...
  e.respondWith(
    caches.match(e.request, { ignoreSearch: true }).then((hit) => {
      // No per-file background refresh: files only change together when a
      // new CACHE_VERSION installs, so index.html and game.js can never be
      // from different releases (that skew broke the admin reset once).
      if (hit) return hit;
